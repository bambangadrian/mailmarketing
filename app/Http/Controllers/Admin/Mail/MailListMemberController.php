...
use MailMarketing\Http\Requests\UpdateMailListMemberRequest;
...
class MailListMemberController extends AbstractAdminController
{
    ...
    public function __construct()
    {
        ...
        $this->contentDir = 'mail/mailList/members';
        # Set page attributes.
        $this->data['pageHeader'] = 'Maillist Members';
        $this->data['pageDescription'] = 'Manage your subscriber on selected mailing list';
        $this->data['activeMenu'] = 'mail';
        $this->data['activeSubMenu'] = 'mailList';
        $this->setReferenceKey('members');
    ...
    public function index($listID = null)
    {
        ...
        $subscriberGroupList = SubscriberGroup::active()
                                              ->notDeleted()
                                              ->where('Sbg_MailListID', $listID)
                                              ->lists('Sbg_ID');
        $this->data['model'] = SubscriberGroupDetail::notDeleted()
                                                    ->with('subscriber', 'subscriberGroup')
                                                    ->whereIn('Sgd_GroupID', $subscriberGroupList)
    ...
    {
        $this->data['listID'] = $listID;
        $this->data['pageDescription'] = 'Add your subscriber to this mailing list';
        ...
        $this->data['subscriberOptions'] = Subscriber::active()
                                                     ->notDeleted()
                                                     ->whereNotExists(
                                                         function ($query) use ($subscriberGroupList) {
     ...
     * @return \Illuminate\Http\Response
     */
    public function edit($listID, $detailID = null)
    {
        $this->data['listID'] = $listID;
        $this->data['pageDescription'] = 'Modify the subscriber list on this mailing list';
        $this->data['indexLinkAction'] = action($this->controllerName.'@index', $listID);
        $this->data['formAction'] = action($this->controllerName.'@update', [$listID, $detailID]);
        $this->data['formDeleteAction'] = action($this->controllerName.'@destroy', [$listID, $detailID]);
        $this->data['model'] = SubscriberGroupDetail::notDeleted()->with('subscriber')->find($detailID);

        return parent::edit($listID);
    }

    /**
     * Store a newly created resource in storage.
     *
     * @param UpdateMailListMemberRequest $request Request object parameter.
     * @param integer                     $listID  Mailing list ID parameter.
     *
     * @return \Illuminate\Http\Response
     */
    public function store(UpdateMailListMemberRequest $request, $listID)
    {
        try {
            $members = [];
            ...
            foreach ($request->get('Sgd_SubscriberID') as $subscriberID) {
                ...
                if (empty($recordSubscriber) === false) {
                    ...
                    $dataInputForSubscriberGroupDetail[] = [
                        'Sgd_GroupID'       => $recordMailList->Mls_DefaultGroupID,
            ...
            return redirect()->action($this->controllerName.'@index', $listID);
        } catch (\Exception $e) {
            \DB::rollback();

            return redirect()
                ->action($this->controllerName.'@create', $listID)
     ...
     * @return \Illuminate\Http\Response
     */
    public function update(UpdateMailListMemberRequest $request, $listID, $detailID)
    {
        $redirectPath = action($this->controllerName.'@edit', [$listID, $detailID]);
        try {
            $recordMailList = MailList::find($listID);
     ...
     * @return \Illuminate\Http\Response
     */
    public function destroy($listID, $detailID = null)
    {
        try {
            $recordMailList = MailList::find($listID);
            ...
            return redirect(action($this->controllerName.'@edit', [$listID, $detailID]))
                ->withErrors($e->getMessage())
                ->withInput();
        }
    }
